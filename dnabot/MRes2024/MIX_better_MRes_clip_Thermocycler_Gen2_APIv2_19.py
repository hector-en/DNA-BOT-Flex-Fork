    ...
    def mix_linkers_function(Mix_linkers_bool, clips_dict, pipette_name, source_plates):
        ...
        if Mix_linkers_bool:
            ...
            for clip_num in range(len(prefixes_unique)):  #high = 2.5, normal = 1, slow = 0.5,  vslow = 0.
                pipette.pick_up_tip()
                pipette.aspirate(linker_vol/2, source_plates[prefixes_unique[clip_num, 0]][prefixes_unique[clip_num, 1]].bottom(2), rate=high)
            ...
            for clip_num in range(len(suffixes_unique)):  
                ...
                pipette.aspirate(linker_vol/2, source_plates[suffixes_unique[clip_num, 0]][suffixes_unique[clip_num, 1]].bottom(2), rate=high)
    ...
    def mix_parts_function(Mix_parts_bool, clips_dict, pipette_name, source_plates):
        ...
        if Mix_parts_bool:
            ...
            for clip_num in range(len(parts_unique)):
                ...
                pipette.aspirate(part_vol/2, source_plates[parts_unique[clip_num, 0]][parts_unique[clip_num, 1]].bottom(2), rate=high)
